...
This repository is an **older AGP codebase** where application packaging is APK-centric (`Dex` -> `PackageApplication` -> optional `ZipAlign`), and there is no first-class App Bundle (`.aab`) pipeline implementation in this branch.
...
## 3) AAB status for this repo

- No modern bundle task chain (`bundle<Variant>`, `PackageBundleTask`, `BundleTool` integration) is present.
- Any AAB strategy here is architectural guidance for newer AGP, not something directly runnable in this source tree.
